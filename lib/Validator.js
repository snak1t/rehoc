  ...
  return function (Component) {
    return function (_React$Component) {
      ...
      _createClass(Validator, [{
        ...
        value: function performAsyncValidation(validator, key, value, state) {
          var _this5 = this;

          var passedCallback = function passedCallback(result) {
            if (result) return;
            return _this5.setState(function (prevState) {
              var stateSlice = _extends({}, prevState[key], {
          ...
          return validator.withFields ? validator.rule.apply(validator, [value].concat(_toConsumableArray(validator.withFields.map(function (key) {
            return state[key].value;
          })), [passedCallback])) : validator.rule(value, passedCallback);
        }
      }, {
        ...
        value: function collectErrors(_ref3) {
          ...
          var failedValidator = validators.find(function (validator) {
            return validator.async ? _this6.handleAsyncValidation({
              ...
              isTarget: isTarget,
              state: state
            }) : !_this6.handleSyncValidation({ validator: validator, key: key, value: value, state: state });
          });
          return failedValidator ? failedValidator.multiple ? [].concat(_toConsumableArray(failedValidator.message)) : [failedValidator.message] : [];
        }
      }, {
        ...
        value: function valueHandler(key) {
          ...
          return function (data) {
            ...
            _this8.setState(function (previousState) {
              var st = _this8.updateStateValue(key, value, true, previousState);
              return st;
            });
          };
        }
