...
var _ValueDescriptor = require('./components/ValueDescriptor');

var VD = _interopRequireWildcard(_ValueDescriptor);
...
class Validator extends _react2.default.Component {
  ...
  constructor(props) {
    super(props);
    this.parseConfig = (0, _compose2.default)((0, _mapRecursively.mapRecursively)(FormHelper.isFormItem, (cfg, _, compoundKey) => cfg.setHandler(this.valueHandler(compoundKey))), ConfigUtility.parseConfig, (0, _path2.default)(['config']));
    ...
    this.setErrorsAndStatus = ({ valueDescriptor, errors }) => formItem => formItem.setErrorsAndStatus(valueDescriptor, errors);
    ...
    this.updateStateValue = ({ valueDescriptor, previousState }) => {
      ...
      const lens = (0, _lensPath2.default)(VD.key(valueDescriptor));
      ...
      return dependencies.length === 0 ? [updatedState, asyncErrors] : dependencies.reduce(this.dependencyStateReducer, [updatedState, asyncErrors]);
    };

    this.dependencyStateReducer = ([state, asyncErrors], key) => {
  ...
  runAsync(asyncErrors, valueDescriptor) {
    Promise.all(asyncErrors).catch(errors => {
      ...
      this.setState((0, _evolve2.default)({ config: FormHelper.mergeWithoutField(updatedState) }));
    });
  }
