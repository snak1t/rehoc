...
class Validator extends _react2.default.Component {
    ...
    constructor(props) {
        ...
        this.updateStateValue = async ({ key, value, isTarget, previousState }) => {
            const errors = await FormHelper.performValidation(previousState, key, value, this.props.config[key].validators, isTarget);
            ...
            const updatedStateSlice = _extends({}, stateSlice, {
                errors,
                value,
                status: {
                    dirty,
            ...
            return !updatedState[key].dependency ? updatedState : updatedState[key].dependency.reduce((state, key) => this.updateStateValue({
                key,
