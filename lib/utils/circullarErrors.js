...
const createDependencyMatrix = exports.createDependencyMatrix = state => {
  const keys = Object.keys(state).sort();

  return keys.map(key => keys.map(innerKey => !!state[key].dependency && state[key].dependency.indexOf(innerKey) !== -1));
};

const createError = (indexes, state) => {
  const fields = Object.keys(state).sort().filter((_, i) => indexes.indexOf(i) !== -1);
  fields.push(fields[0]);
  throw new Error('Found a circular dependency in validation config for ' + fields.join(' -> ') + ' -> ...');
};

const findCircularDependency = exports.findCircularDependency = _state => {
  const state = flatten(_state);
  const matrix = createDependencyMatrix(state);
  const helper = (index, matrix, stack) => matrix[index].reduce((acc, item, index) => item ? [...acc, index] : acc, []).forEach((item, index) => {
