...
const performValidation = exports.performValidation = ({ state, valueDescriptor }) => {
  const stateField = (0, _path2.default)(valueDescriptor.key, state);
  if (skipValidation(stateField, valueDescriptor.isTarget, valueDescriptor.value)) {
    return [[], []];
  }

  const getOtherFields = v => {
    return v.withFields ? v.withFields.map(k => (0, _path2.default)([...k.split('.'), 'value'], state)) : [];
  };
...
const mergeWithoutField = exports.mergeWithoutField = (objWithValues, objWOValues) => (0, _mapRecursively.mapRecursively)((0, _has2.default)('value'), (element, _, _key) => {
  const key = _key.split('.');
  const keyPath = (0, _path2.default)(key);
  if (element === keyPath(objWithValues)) {
    return element;
  }
  return _extends({}, keyPath(objWithValues), element, {
    value: (0, _path2.default)([...key, 'value'], objWithValues)
  });
})(objWOValues);
